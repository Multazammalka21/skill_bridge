...
<div style="max-width:680px;">
    <div style="margin-bottom:24px;">
        ...
        <p style="color:var(--text-muted); font-size:0.85rem;">Contoh: Literasi, Numerasi, Pengenalan Lingkungan, Sains Dasar</p>
    </div>
        ...
        <div class="card__body">
            <form action="{{ route('admin.categories.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="nama" class="form-input" placeholder="Contoh: Literasi, Numerasi, Pengenalan Lingkungan"
                ...
                <div class="form-group">
                    ...
                    <textarea name="deskripsi" class="form-textarea" placeholder="Deskripsi singkat kategori pembelajaran ini..." maxlength="500">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi') <div style="color:var(--red); font-size:0.78rem; margin-top:4px;">{{ $message }}</div> @enderror
                </div>

                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                    <div class="form-group">
                        <label class="form-label">Ikon (Emoji) <span style="color:var(--red);">*</span></label>
                        <input type="text" name="ikon" class="form-input" placeholder="📚"
                               value="{{ old('ikon', '📚') }}" required maxlength="10"
                               style="font-size:1.5rem; text-align:center;">
                        <div style="font-size:0.75rem; color:var(--text-muted); margin-top:4px;">Masukkan emoji yang mewakili kategori ini</div>
                        @error('ikon') <div style="color:var(--red); font-size:0.78rem; margin-top:4px;">{{ $message }}</div> @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div style="font-size:0.75rem; color:var(--text-muted); margin-top:4px;">Warna aksen untuk tampilan kategori di UI</div>
                        @error('warna') <div style="color:var(--red); font-size:0.78rem; margin-top:4px;">{{ $message }}</div> @enderror
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="urutan" class="form-input" placeholder="0"
                           value="{{ old('urutan', 0) }}" min="0" required>
                    <div style="font-size:0.75rem; color:var(--text-muted); margin-top:4px;">Angka kecil tampil lebih awal. Contoh: 0 = pertama, 1 = kedua, dst.</div>
                    @error('urutan') <div style="color:var(--red); font-size:0.78rem; margin-top:4px;">{{ $message }}</div> @enderror
                </div>

                {{-- Preview card --}}
                <div style="background:#f8fafc; border:1px solid var(--border); border-radius:10px; padding:16px; margin-bottom:20px;">
                    <div style="font-size:0.78rem; font-weight:600; color:var(--text-muted); margin-bottom:12px;">👁️ Preview Kartu Kategori:</div>
