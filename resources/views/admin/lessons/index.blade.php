...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
    <div>
        <h2 style="font-size:1.4rem; font-weight:800; color:var(--text-primary); margin-bottom:4px;">Materi Pembelajaran</h2>
        <p style="color:var(--text-muted); font-size:0.85rem;">Kelola semua lesson dengan konten audio, gambar, animasi, dan teks</p>
    </div>
    ...
    <div class="card__body" style="padding:16px 20px;">
        <form method="GET" action="{{ route('admin.lessons.index') }}" style="display:flex; gap:12px; flex-wrap:wrap; align-items:flex-end;">
            <div class="form-group" style="margin:0; flex:1; min-width:160px;">
                <label class="form-label"><i class="ti ti-search"></i> Cari Judul</label>
                <input type="text" name="search" class="form-input" placeholder="Cari materi..." value="{{ $filters['search'] ?? '' }}">
            </div>
            <div class="form-group" style="margin:0; min-width:150px;">
                <label class="form-label"><i class="ti ti-category"></i> Kategori</label>
            ...
            <div class="form-group" style="margin:0; min-width:130px;">
                <label class="form-label"><i class="ti ti-world"></i> Tipe Dunia</label>
            ...
            <div class="form-group" style="margin:0; min-width:120px;">
                <label class="form-label"><i class="ti ti-user"></i> Usia</label>
            ...
            <div style="display:flex; gap:8px;">
                ...
                <a href="{{ route('admin.lessons.index') }}" class="btn btn--neutral">Reset</a>
            </div>
        </form>
    </div>
</div>
...
<div class="card">
    <div class="card__header">
        <h3 class="card__title"><i class="ti ti-book"></i> Daftar Materi</h3>
        <span style="font-size:0.82rem; color:var(--text-muted);">{{ $lessons->total() }} materi ditemukan</span>
    </div>
    <div class="card__body" style="padding:0;">
        @if($lessons->isEmpty())
            <div style="text-align:center; padding:60px 20px; color:var(--text-muted);">
                ...
                <a href="{{ route('admin.lessons.create') }}" class="btn btn--primary" style="margin-top:16px;">➕ Tambah Materi Pertama</a>
            </div>
        @else
            <table class="admin-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="text-align:center; font-weight:700; color:var(--text-muted); font-size:0.8rem;">{{ $lesson->urutan }}</td>
                        <td>
                            <div style="font-weight:700; color:var(--text-primary); margin-bottom:2px;">{{ Str::limit($lesson->judul, 45) }}</div>
                            @if($lesson->prerequisite_lesson_id)
                                <div style="font-size:0.72rem; color:var(--teal);">🔒 Perlu prasyarat</div>
                            @endif
                            @if($lesson->deskripsi)
                                <div style="font-size:0.75rem; color:var(--text-muted);">{{ Str::limit($lesson->deskripsi, 55) }}</div>
                            @endif
                        </td>
                        <td>
                            @if($lesson->category)
                        ...
                        <td>
                            @if($lesson->kategori_usia === '5-7')
                                <span class="badge badge badge--green">5–7 Thn</span>
                            @else
                                <span class="badge badge badge--blue">8–10 Thn</span>
                            @endif
                        </td>
                        <td>
                            @php
                            ...
                            @endphp
                            <span style="font-size:0.78rem; color:var(--text-secondary);">{{ $kontenIkon }}</span>
                        </td>
                        <td style="font-size:0.85rem; color:var(--text-secondary);">{{ $lesson->durasi_menit }} mnt</td>
                        <td>
                            @if($lesson->aktif)
                        ...
                        <td>
                            <div style="display:flex; gap:5px;">
                                <a href="{{ route('admin.lessons.edit', $lesson) }}" class="btn btn--neutral btn--sm" title="Edit"><i class="ti ti-edit"></i></a>
                                <form action="{{ route('admin.lessons.destroy', $lesson) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn--danger btn--sm" title="Hapus"><i class="ti ti-trash"></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
