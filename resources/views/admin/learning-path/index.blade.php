...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
    <div>
        <h2 style="font-size:1.4rem; font-weight:800; color:var(--admin-text-primary); margin-bottom:4px;">🗺️ Learning Path</h2>
        <p style="color:var(--admin-text-muted); font-size:0.85rem;">Atur urutan lesson dan prasyarat — anak harus menyelesaikan materi sebelumnya untuk membuka level berikutnya</p>
    </div>
    <button id="saveBtn" class="admin-btn admin-btn--success" onclick="savePath()" style="display:none;">
        💾 Simpan Urutan
    </button>
</div>
...
<div class="admin-card" style="margin-bottom:20px;">
    <div class="admin-card__body" style="padding:16px 20px;">
        <form method="GET" action="{{ route('admin.learning-path.index') }}" style="display:flex; gap:12px; flex-wrap:wrap; align-items:flex-end;">
            ...
            <div class="admin-form-group" style="margin:0; min-width:140px;">
...
@if($lessons->isEmpty())
    <div style="text-align:center; padding:80px 20px; background:#fff; border-radius:12px; border:1px solid var(--admin-border);">
        ...
        <p style="color:var(--admin-text-muted); font-size:0.88rem; margin-bottom:20px;">Tambahkan materi pembelajaran terlebih dahulu atau ubah filter di atas.</p>
        <a href="{{ route('admin.lessons.create') }}" class="admin-btn admin-btn--primary">➕ Tambah Materi Baru</a>
    </div>
@else
    <div class="admin-card">
        <div class="admin-card__header">
            <h3 class="admin-card__title">📋 Urutan Pembelajaran</h3>
            <div style="display:flex; align-items:center; gap:12px;">
                <span style="font-size:0.8rem; color:var(--admin-text-muted);">{{ $lessons->count() }} materi</span>
                <span style="font-size:0.78rem; background:rgba(59,130,246,0.1); color:var(--admin-primary); padding:3px 10px; border-radius:20px; font-weight:600;">
        ...
        <div class="admin-card__body" style="padding:0;">
            ...
            <div id="sortableList" style="padding:16px; display:flex; flex-direction:column; gap:10px;">
                ...
                <div class="path-item" data-id="{{ $lesson->id }}" data-urutan="{{ $idx }}"
                     style="background:#fff; border:2px solid var(--admin-border); border-radius:12px; padding:16px 18px; cursor:grab; transition:all 0.15s; display:flex; align-items:center; gap:14px; user-select:none;">

                    {{-- Drag handle --}}
                    <div class="drag-handle" style="color:#cbd5e1; font-size:1.2rem; cursor:grab; flex-shrink:0;">⣿</div>

                    {{-- Step number --}}
                    <div style="width:34px; height:34px; border-radius:50%; background:var(--admin-primary); color:#fff; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:0.88rem; flex-shrink:0;" class="step-num">
                        {{ $idx + 1 }}
                    </div>

                    {{-- Lesson info --}}
                    <div style="flex:1; min-width:0;">
                        <div style="font-weight:700; color:var(--admin-text-primary); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                            {{ $lesson->judul }}
                        </div>
                        <div style="display:flex; gap:8px; margin-top:4px; flex-wrap:wrap;">
                            ...
                            @endif
                            <span style="font-size:0.7rem; color:var(--admin-text-muted);">{{ $lesson->durasi_menit }} menit</span>
                            @if($lesson->aktif)
                    ...
                    <div style="flex-shrink:0; min-width:220px;">
                        <label style="font-size:0.72rem; font-weight:600; color:var(--admin-text-muted); display:block; margin-bottom:4px;">🔒 Prasyarat:</label>
                        <select class="admin-form-select prereq-select" data-lesson-id="{{ $lesson->id }}"
            ...
            <div style="padding:16px 20px; border-top:1px solid var(--admin-border); display:flex; justify-content:flex-end; gap:12px;">
                <button class="admin-btn admin-btn--ghost" onclick="location.reload()">↺ Reset</button>
                <button class="admin-btn admin-btn--success" onclick="savePath()">💾 Simpan Urutan & Prasyarat</button>
            </div>
        </div>
    </div>
@endif
...
<style>
.path-item.dragging { opacity:0.4; border-color:var(--admin-primary); }
.path-item.drag-over { border-color:var(--admin-primary); background:rgba(59,130,246,0.04); }
</style>
...
if (list) {
    ...
    list.addEventListener('dragstart', e => {
        ...
        placeholder.style.cssText = `height:${dragEl.offsetHeight}px; border:2px dashed var(--admin-primary); border-radius:12px; background:rgba(59,130,246,0.04); margin-bottom:0;`;
